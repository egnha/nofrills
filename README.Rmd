...
variation of `function()` that:
  ...
  - **costs less** — enables tidyverse
    ...
    [functional impurity](#leveraging-quasiquotation)
...
```
(Source:
[_Programming with dplyr_](http://dplyr.tidyverse.org/articles/programming.html))
...
This is because a function may depend on mutable objects in its
[lexical scope](http://adv-r.hadley.nz/functions.html#lexical-scoping). Normally
this isn’t an issue. But if you are working interactively and sourcing files
into the global environment, say, it can be tricky to ensure that you
